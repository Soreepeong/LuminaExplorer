...
public class ValueString : IValue {
    ...
    internal static ValueArray ReadVector(Parser parser, int count) => new(
        Enumerable.Range(0, count).Select(_ => (IValue?) Read(parser)).ToImmutableList(),
        FieldType.SingleString);
}
